 ...
 */
internal object ProfanityDetector {
    private val BANNED_TERMS = arrayOf("spring", "nodejs", "vertx")
     ...
     */
    fun detectProfanity(message: String?) {
        var message = message ?: return
        message = message.toLowerCase()
        for (bannedTerm in BANNED_TERMS) {
            if (message.contains(bannedTerm)) {
                throw ProfanityException(bannedTerm)
            }
        }
    }
}