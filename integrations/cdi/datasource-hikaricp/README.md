...
With Docker:
```bash
docker build -t helidon-kotlin-examples-integrations-datasource-hikaricp .
docker run --rm -d \
    --link oracle \
    -e javax_sql_DataSource_example_dataSource_url="jdbc:oracle:thin:@oracle:1521:ORCL" \
    --name helidon-kotlin-examples-integrations-datasource-hikaricp \
    -p 8080:8080 helidon-kotlin-examples-integrations-datasource-hikaricp:latest
```
OR
...
```bash
mvn package
java -jar target/helidon-kotlin-examples-integrations-datasource-hikaricp.jar
```
...
```bash
docker stop oracle helidon-kotlin-examples-integrations-datasource-hikaricp
```
