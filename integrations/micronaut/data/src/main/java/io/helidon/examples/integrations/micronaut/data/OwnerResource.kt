 ...
 */ @Inject constructor(private val ownerRepository: DbOwnerRepository) {
    ...
    @GET
    @SimplyTimed
    open fun owner(@PathParam("name") name: @Pattern(regexp = "\\w+[\\w+\\s?]*\\w") String?): Owner {
        return ownerRepository.findByName(name)
            ?.orElseThrow { NotFoundException("Owner by name $name does not exist") }!!
    }
}